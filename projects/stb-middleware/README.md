...
**STB Middleware 정합/개발**은 2012년부터 약 10년간 수행한 케이블 TV 셋톱박스 미들웨어 개발 및 단말 정합 프로젝트의 총체입니다.

다수의 MSO(복수종합유선방송사업자) — TBroad, 제주방송, LGHV, 딜라이브, 충북방송(CCS), KCTV, HCN, JCN 등 — 을 대상으로, 신규 단말 정합부터 UI/UX 서비스 개발, CAS/XCAS 전환, 클라우드 서비스 구축까지 30여 건의 공식 프로젝트를 수행했습니다.

### 업계 배경

본 프로젝트들은 국내 방송 미들웨어 전문 기업의 솔루션을 기반으로 수행되었습니다. **알티캐스트**(Alticast, 1999년 설립)는 DVB-MHP, OCAP, ACAP 기반 미들웨어를 세계 최초로 상용화한 기업으로, 전 세계 25개 이상의 유료방송 사업자에게 5,000만 대 이상의 디바이스에 소프트웨어 플랫폼을 공급하고 있습니다. 2019년 물적분할로 **알티미디어**가 설립되어 국내 방송 미들웨어 사업을 이어받았으며, 2025년 **KT 알티미디어**로 사명이 변경되었습니다.

UI/UX 플랫폼은 **Windmill**(인터랙티브 클라이언트-서버 시스템) → **Wind3/Command Cloud** → **Image Cloud**(클라우드에서 UI를 렌더링하고 STB에 경량 프레젠테이션 레이어를 전송하는 방식)로 세대가 진화했으며, 이 과정에서 기존 레거시 단말과 신규 클라우드 서비스의 공존을 미들웨어 레벨에서 해결하는 것이 핵심 과제였습니다.
...
> **OCAP** (OpenCable Application Platform) — CableLabs에서 정의한 Java 기반 케이블 STB 미들웨어 표준. DVB-MHP 사양 기반으로 북미 케이블 환경에 맞게 확장.
> **MHP** (Multimedia Home Platform) — DVB 프로젝트의 인터랙티브 디지털 TV용 오픈 미들웨어 표준 (ETSI TS 102 796). Java VM 기반으로 하드웨어 독립적 애플리케이션 실행 환경 제공.
> **XCAS** (eXchangeable CAS) — 다운로드 가능한 제한수신 시스템. 양방향 케이블 네트워크를 통해 CAS 소프트웨어를 다운로드·갱신하는 한국 표준 규격.
> **DVB-SI** (Service Information) — DVB 비트스트림의 서비스 정보 표준 (ETSI EN 300 468). EPG 및 수신기 자동 튜닝 지원.
